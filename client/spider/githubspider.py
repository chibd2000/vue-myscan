...
from core.exception.github import GithubPrivilegeError, GithubTokenError
...
class GithubSpider(BaseSpider):
    ...
    async def github_search(self, session, page):
        ...
        text = await AsyncFetcher.fetch(session=session, url=self.addr.format(self.domain, page, self.per_page), timeout=self.reqTimeout, headers=headers, json=True)
        if 'API rate limit exceeded' in str(text):
            raise GithubPrivilegeError("github api rate limit") from None
        elif 'Validation Failed' in str(text):
            raise GithubTokenError("github token error") from None
        return text

    async def get_subdomains(self, session, url):
        text = await AsyncFetcher.fetch(session=session, url=url, timeout=self.reqTimeout, headers=self.headers)
    ...
    async def spider(self):
        ...
        try:
            ...
            gLogger.myscan_error('curl github.com api rate limit')
            return []
