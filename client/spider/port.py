...
class PortScan(BaseSpider):
    ...
    async def quick_scan(self, semaphore, ip, port):
        async with semaphore:
            ...
            if result is not None:
                self.res_list.append({'id': 'NULL', 'domain_id': self.name, 'ip': str(result.get('ip')), 'port': int(result.get('port')), 'service': str(result.get('service')), 'title': str(result.get('title')), 'info': str(result.get('versioninfo'))})
                flag = True
    ...
    async def spider(self):
        ...
        #     {'ip': str(result.get('ip')), 'port': str(result.get('port')), 'service': str(result.get('service')),
        #      'title': str(result.get('title')), 'versioninfo': str(result.get('versioninfo'))})
