        ...
        for page in range(1, self.PAGES + 1):
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    text = await AsyncFetcher.fetch(session=session, url=self.addr.format(quote(word + " site:*." + self.domain), first_value), headers=self.headers, timeout=self.reqTimeout)
                    ...
                    for li in lis:
                        ...
                        web_info = {'id': 'NULL', 'domain_id': self.name, 'keyword': word, 'url': link, 'title': title,
                                    'type': self.source}
