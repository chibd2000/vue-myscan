...
class AliveSpider(BaseSpider):
    ...
    async def _getAlive(self, semaphore):
        ...
        try:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(url, headers=self.headers, verify_ssl=False, timeout=60) as response:
                        if response is not None:
                            ...
                            title = self._get_title(BeautifulSoup(text, 'lxml')).replace('\n','')
                            status = response.status
                            header = response.headers.get('X-Powered-By', '')
                            self.res_list.append({'id': 'NULL', 'domain_id': self.name, 'url': url, 'status': status, 'title': title, 'header': header})
            ...
            self.res_list.append({'id': 'NULL', 'domain_id': self.name, 'url': url, 'status': '手动访问', 'title': '-', 'header': '-'})
    ...
    # @db_update_status('domain_webtitle')
