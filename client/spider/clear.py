...
class ClearSpider(BaseSpider):
    ...
    def flush_ip_segment(self, ip_list, ip_segment_list):
        ...
        for ip_segment in temp_ip_segment_list:
            ip_segment_list.append({'id': 'NULL', 'domain_id': self.name, 'ip_segment': ip_segment, 'ips': [], 'num': 0})

        for ip in filter_ip_list:
            for index, ip_segment in enumerate(ip_segment_list):
                if ip_in_subnet(ip, ip_segment['ip_segment']):
                    ip_segment_list[index]['num'] += 1
                    ip_segment_list[index]['ips'].append(ip)

        for index, ip_segment in enumerate(ip_segment_list):
            ip_segment_list[index]['ips'] = ','.join(ip_segment_list[index]['ips'])

        # self.write_file(ip_segment_list, 5)
        self.tb_name = 'domain_cseg'
        self.write_db(ip_segment_list)
        ...
        for asn in asn_list:
            ...
            for filter_asn in FILTER_CDN_ASN_LIST:
                if str(asn) == str(filter_asn):
                    ...
                    break
            if flag:
                filter_asn_list.append({'id': 'NULL', 'domain_id': self.name, 'asn': int(asn)})
        # self.write_file(filter_asn_list, 6)
        self.tb_name = 'domain_asn'
        self.write_db(filter_asn_list)
        self._is_continue = False
