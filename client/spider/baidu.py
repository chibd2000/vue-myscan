...
class BaiduSpider(BaseSpider):
    ...
    async def keyword(self, session, kw, page=1):
        try:
            text = await AsyncFetcher.fetch(session=session, url=self.addr.format(quote(kw), page), headers=self.headers, timeout=self.reqTimeout)
    ...
    async def location(self, session, link):
        try:
            response = await AsyncFetcher.fetch_response(session=session, url=link, headers=self.headers, timeout=self.reqTimeout)
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    ...
                    for _ in redirect_link_list:
                        ...
                        web_info = {'id': 'NULL', 'domain_id': self.name, 'keyword': word, 'url': link, 'title': title, 'type': self.source}
    ...
    async def spider(self):
        ...
        self.write_db(self.get_unique_list(self.web_list))
