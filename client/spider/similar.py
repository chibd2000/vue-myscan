...
class SimilarSpider(BaseSpider):
    def __init__(self, domain, name, domain_list):
        ...
        self.name = name
        self.source = 'Similar'
        self.tb_name = 'domain_similar'
        self.res_list = self.get_similarity_match(self.domain, domain_list)

    def get_similarity_match(self, domain, domain_list):
        result_list = []
        ...
        while domain_index < len(new_domain_list):
            ...
            result_list.append({'id': 'NULL', 'domain_id': self.name, 'similar': ','.join(current_result_list)})
            domain_index += 1
        return result_list
    ...
    async def spider(self):
        ...
        self.write_db(self.get_unique_list(self.res_list))
