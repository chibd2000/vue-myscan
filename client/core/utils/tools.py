...
"""
>>> def sql_format(sql_field, *args):
...     print(args)
...     if 'is_string' in args:
...         return '"{}"'.format(sql_field)
...     return str(sql_field)
...
>>>
>>> sql_format('ip', 'is_string')
('is_string',)
'"ip"'
>>> sql_format('ip', 'is_int')
('is_int',)
'ip'
"""
def sql_format(sql_field, *args):
...
def is_subdomain(subdomain, domain):
    """判断是否为子域名，因为信息搜集中难免会出现一些非预期的域名数据，这里的话还是需要进行过滤"""
