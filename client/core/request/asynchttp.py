...
class AsyncFetcher:
    ...
    async def fetch(session, url, params=None, json=False, *args, **kwargs) -> Union[str, dict, list]:
        try:
            if params is None:
                async with session.get(url, verify_ssl=False, proxy=config_dict['proxy'], *args, **kwargs) as response:
                    if response is not None:
                        await asyncio.sleep(2)
                        return await response.text() if json is False else await response.json()
            else:
                async with session.get(url, verify_ssl=False, proxy=config_dict['proxy'], params=params, *args, **kwargs) as response:
                    if response is not None:
                        await asyncio.sleep(2)
                        return await response.text() if json is False else await response.json()
    ...
    @staticmethod
    async def fetch_response(session, url, params=None, *args, **kwargs) -> Union[aiohttp.ClientResponse, None]:
        try:
            if params is None:
                async with session.get(url, verify_ssl=False, proxy=config_dict['proxy'], *args, **kwargs) as response:
                    if response is not None:
    ...
    async def post_fetch(session, url, data=None, params=None, json=False, *args, **kwargs):
        try:
            if params is None:
                async with session.post(url=url, verify_ssl=False, proxy=config_dict['proxy'], data=data, *args, **kwargs) as response:
                    if response is not None:
                        await asyncio.sleep(2)
                        return await response.text() if json is False else await response.json()
            else:
                async with session.post(url=url, verify_ssl=False, proxy=config_dict['proxy'], data=data, params=params, *args, **kwargs) as response:
                    if response is not None:
                        await asyncio.sleep(2)
                        return await response.text() if json is False else await response.json()
        except Exception:
