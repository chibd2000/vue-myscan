...
from spider.similar import SimilarSpider
...
def ksubdomain(domain, task_name):
    gLogger.myscan_debug("ksubdomain_spider_start")
    GlobalVariableManager.get_value('g_db').update_task_module_status('ksubdomain', task_name)
    ksub_domain_list = []
...
class TargetManager:
    def __init__(self, task_name, target):
        ...
        self.g_db = GlobalVariableManager.get_value('g_db')
    ...
    def write_db(self, data, tb_name):
        g_db = GlobalVariableManager.get_value('g_db')
        [g_db.insert(tb_name, _) for _ in data]
    ...
    def _register_target_options(self, target):
        ...
        if target.proxy:
            if 'http' not in target.proxy:
                gLogger.myscan_warn('if you want to set proxy, the format need -x http://...')
                exit(0)
        ...
        if target.port:
            GlobalVariableManager.set_value('port_config', target.port)
    ...
    async def _domain_third_engine_api(self, target_domain):
        ...
        self.g_db.update_task_module_status('ThirdSpider', self.task_name)
    ...
    async def _domain_search_engine_api(self, target_domain):
        ...
        self.g_db.update_task_module_status('SearchSpider', self.task_name)
        task_list = [
    ...
    async def _domain_space_engine_api(self, target_domain):
        ...
        self.g_db.update_task_module_status('WebSpaceSpider', self.task_name)
    ...
    async def _friend_search_api(self, target_domain):
        ...
        self.g_db.update_task_module_status('FriendSpider', self.task_name)
    ...
    async def _domain_2_ip_api(self, target_domain):
        ...
        self.g_db.update_task_module_status('domain2ipSpider', self.task_name)
    ...
    async def _ip_2_domain_api(self, target_domain):
        ...
        self.g_db.update_task_module_status('ip2domainSpider', self.task_name)
    ...
    async def _ssl_search_api(self, target_domain):
        ...
        self.g_db.update_task_module_status('SSLSpider', self.task_name)
    ...
    async def _port_scan_api(self, target_domain):
        ...
        self.g_db.update_task_module_status('PortSpider', self.task_name)
        port_config = GlobalVariableManager.get_value('port_config')
    ...
    async def _alive_search_api(self, target_domain):
        ...
        self.g_db.update_task_module_status('AliveSpider', self.task_name)
        self.domain_alive_list = await AliveSpider(target_domain, self.task_name, self.domain_list).main()

    async def _similar_search_api(self, target_domain):
        gLogger.myscan_debug("_similar_spider_start")
        self.g_db.update_task_module_status('SimilarSpider', self.task_name)
        await SimilarSpider(target_domain, self.task_name, self.domain_list).main()

    async def _search_func(self, target_domain):
        if self.target_ksub_domain:
            self.domain_list.extend(ksubdomain(target_domain, self.task_name))
        ...
        await self._similar_search_api(target_domain)
    ...
    async def scan(self, loop=None):
        ...
        result_list = []
        self.g_db.update_task_module_status('BugChecker', self.task_name)
        ...
        self.write_db(result_list, 'domain_vuln')
