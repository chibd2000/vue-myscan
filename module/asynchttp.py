...
class AsyncFetcher:
    ...
    @staticmethod
    async def fetch_response(session, url, params=None, *args, **kwargs) -> ClientResponse | None:
        try:
            if params is None:
                response = await session.get(url, verify_ssl=False, *args, **kwargs)
                if response is not None:
                    return response
            else:
                response = await session.get(url, verify_ssl=False, params=params, *args, **kwargs)
                if response is not None:
                    return response
        except Exception as e:
            return None
