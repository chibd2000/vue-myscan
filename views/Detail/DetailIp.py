        ...
        if request.method == 'GET':
            if query == 'null':
                detail_ip = DomainIp.query.outerjoin(DomainService, DomainIp.ip == DomainService.ip).with_entities(
                    DomainIp.ip, func.group_concat(DomainService.port), DomainIp.ip2domain).filter_by(
                    domain_id=current_domain_id).group_by(DomainIp.ip)
            else:
                if search_type == 'ip':
                    detail_ip = DomainIp.query.outerjoin(DomainService, DomainIp.ip == DomainService.ip).with_entities(
                        DomainIp.ip, func.group_concat(DomainService.port), DomainIp.ip2domain).filter_by(
                        domain_id=current_domain_id, ip=query).group_by(DomainIp.ip)
